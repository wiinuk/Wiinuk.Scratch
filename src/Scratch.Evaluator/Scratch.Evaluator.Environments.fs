...
module internal Internals =
    ...
    let stackMessage state location e =
        ...
        |> List.map (fun (StackFlame(location, script, arguments, self)) ->
            ...
            let arguments = arguments |> Map.toSeq |> Seq.map (fun (k, v) -> $"{k}=%A{v}") |> String.concat ","

            $"\r\n{state.blockState.config.showState location}: {self.shared.objectName}.{name} {{%A{arguments}}}"
        )
        |> String.concat ""
    ...
    let getListOrRaise (state: _ inref) location name =
        ...
        if state.blockState.config.useVariableDefinitionCheck then
            cfailwithf state location $"list '{name}' not found"
        else
            // TODO:
            ResizeArray()
    ...
    let getValueOrRaise (state: _ inref) location name =
        ...
        if state.blockState.config.useVariableDefinitionCheck then
            cfailwithf state location $"variable '{name}' not found"
        else
            // TODO:
            ref (SString "")
    ...
    let getProcedureOrRaise (state: _ inref) location name =
        ...
        | ValueNone -> cfailwithf state location $"procedure '{name}' not found"
        | ValueSome proc -> proc
    ...
    let setCloudValue (state: _ inref) location name v =
        if not <| SValue.isCloudValue v then
            if state.blockState.config.useCloudValueCheck then
                cfailwithf state location $"invalid cloud value: serVar:to:, '{name}' '%A{v}'"
        else
